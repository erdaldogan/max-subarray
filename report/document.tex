...
Another solution is by just traversing the array once and while traversing the array, comparing the current element with the sum of the current element with previous elements’ sum and since the problem is finding the maximum contiguous subarray, we pick the larger value when comparing. This algorithm is also known as Kadane’s Algorithm.
\begin{algorithm}
	\caption{Linear Time}
	\begin{algorithmic}[1]
